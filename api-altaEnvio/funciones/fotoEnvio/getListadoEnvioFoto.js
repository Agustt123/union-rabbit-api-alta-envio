...
async function getListadoEnvioFoto(
  connection,
  { fechaDesde, fechaHasta, pagina = 1, cantidad = 10, choferes, direccion } // ← opcional
) {
  try {
    const hoy = new Date();
    ...
    // ---- filtro direccion: SOLO address_line y NO toca el CASE de "direccion"
    let direccionFilter = '';
    let direccionParams = [];
    if (typeof direccion === 'string' && direccion.trim() !== '') {
      const terms = direccion.trim().split(/\s+/).filter(Boolean);
      if (terms.length > 0) {
        const ADDR_EXPR = `edd.address_line COLLATE utf8mb4_general_ci`;
        direccionFilter = 'AND ' + terms.map(() => `(${ADDR_EXPR} LIKE ?)`).join(' AND ');
    ...
    const dataQuery = `
      ...
      WHERE e.elim = 69 AND e.superado = 0
        ...
        AND e.autoFecha BETWEEN ? AND ?
        ${choferFilter}
        ${direccionFilter}   -- ← solo afecta el WHERE, no el CASE
      GROUP BY e.did
      ORDER BY e.did DESC
      LIMIT ? OFFSET ?
    `;
